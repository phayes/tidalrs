...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let new_playlist = client.create_playlist(
        "TidalRS Test Playlist",
        "A test playlist created with TidalRS"
    ).await?;
    
    println!("Created playlist: {} ({})", new_playlist.title, new_playlist.uuid);
    ...
    let track_ids: Vec<u64> = search_results.tracks.items.iter()
    ...
    let etag = playlist_with_etag.etag.as_ref()
    ...
    println!("\nTracks in playlist:");
    let playlist_tracks = client.playlist_tracks(&new_playlist.uuid, Some(0), Some(50)).await?;
    ...
    if !playlist_tracks.items.is_empty() {
        ...
        let fresh_etag = fresh_playlist.etag.as_ref()
            .ok_or("Playlist etag not found")?;

        client.remove_track_from_playlist(
            &new_playlist.uuid,
            fresh_etag,
            track_to_remove
        ).await?;
    ...
    let updated_tracks = client.playlist_tracks(&new_playlist.uuid, Some(0), Some(50)).await?;
