...
use tidalrs::{TidalClient, Authz, Order, OrderDirection};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let favorite_tracks = client.favorite_tracks(
        Some(0),    // offset
        Some(10),   // limit
        Some(Order::Date),
    ...
    for (index, fav_track) in favorite_tracks.items.iter().enumerate() {
        println!("  {}. {} by {} (added: {})", 
            ...
            fav_track.item.artists.first().map(|a| a.name.as_str()).unwrap_or("Unknown"),
            fav_track.created
        );
    }
    ...
    let favorite_albums = client.favorite_albums(
        Some(0),    // offset
        Some(10),   // limit
        Some(Order::Date),
        Some(OrderDirection::Desc)
    ...
    for (index, fav_album) in favorite_albums.items.iter().enumerate() {
        println!("  {}. {} by {} (added: {})", 
            ...
            fav_album.item.artists.first().map(|a| a.name.as_str()).unwrap_or("Unknown"),
            fav_album.created
        );
    }
    ...
    let favorite_artists = client.favorite_artists(
        Some(0),    // offset
        Some(10),   // limit
        Some(Order::Date),
        Some(OrderDirection::Desc)
